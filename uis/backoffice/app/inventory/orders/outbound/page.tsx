...
import { getInventoryProducts, createOutboundOrder } from '../../../../lib/inventory';
 ...
 */
function OutboundOrderContent(): React.ReactElement {
  ...
  const handleSubmitOutboundOrder = async (payload: OutboundOrderPayload): Promise<void> => {
    ...
    try {
      ...
      setSuccessMessage(
        `Outbound order #${result.id} successfully registered! Dispatched ${result.quantity} unit(s) of ${result.sku_code}.`
      );
      // Refresh products list so updated current_stock is reflected reactively
      await fetchProductsList();
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'An error occurred while creating the outbound order.';
      setApiError(message);
    } finally {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center p-16 space-y-4 rounded-xl border border-slate-800 bg-slate-900/40">
          ...
          <p className="text-sm font-medium text-slate-400">Loading products for outbound order...</p>
        </div>
      ) : (
        <OutboundOrderForm
          products={products}
          initialSkuId={initialSkuId}
          onSubmit={handleSubmitOutboundOrder}
...
/**
 * Outbound Order Page Component wrapped in Suspense for Next.js searchParams hydration.
 *
 * @returns JSX Element rendering the Outbound Order form page.
 */
export default function OutboundOrderPage(): React.ReactElement {
  return (
